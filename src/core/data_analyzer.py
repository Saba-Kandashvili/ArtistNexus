# src/core/data_analyzer.py (Corrected Version)
...
class DataAnalyzer(BaseManager):
    ...
    def load_data(self):
        """
        Loads all data from the 'artists' table. The aggressive dropna has been removed.
        """
        try:
    ...
    def get_top_n_countries_by_followers(self, n=15):
        ...
        # Perform cleaning specific to this analysis
    ...
    def get_most_popular_artist_in_country(self, country: str):
        if self.df.empty: return None
        analysis_df = self.df.dropna(subset=['country', 'spotify_popularity', 'image_url', 'spotify_url'])
        if country not in analysis_df['country'].values: return None

        country_df = analysis_df[analysis_df['country'] == country]
        if country_df.empty: return None
